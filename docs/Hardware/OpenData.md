...
OpenData is a lightweight communication framework designed for WiFi-based Solarman IoT devices (such as sockets, energy meters, meter readers, etc.). Devices connect to the local network via WiFi, supporting both data push and external query response mechanisms.
...
|---------------------|---------------------------------------|----------------|
| P1 Meter Reader     | Address: 255.255.255.255; Port: 8088  | 500            |
| Smart Metering Socket | Address: 255.255.255.255; Port: 8088  | 500            |
| Smart DIN Rail Meter | Address: 255.255.255.255; Port: 8088  | 200            |
...
3. Use online AES tools to generate Base64-encoded password and Tag:
   - **Required Tools**:
     ASCII to Hex, Hex to Base64, AES_GCM Encryptor
   - **Parameters**:
     ...
     - Nonce: Random value (Hex, pad to 12 bytes).
...
Parameter	Type	Description
config	object	Only hostname and timezone are writable. See Config.
...
timezone	int	Timezone offset (minutes)	R/W
...
Parameter	Type	Description
config	object	See Config. Sets socket status.
...
Active power	float	Active power (kW)	R
...
- Response: JSON-formatted meter data.
...
active Power	Forward/Reverse active power (kW)
apparent power	Forward/Reverse apparent power (kVA)
reactive power	Forward/Reverse reactive power (kVAR)
total_act_energy	Total forward active energy (kWh)
total_act_ret_energy	Total reverse active energy (kWh)

## 6. Devices
#### P1 Meter Reader
The P1 Meter Reader (P1-2W) connects directly to a single P1 meter via RJ12 interface, continuously collecting meter status and electricity data for long-term monitoring. It transmits data over WiFi to local/cloud platforms, visualizing real-time status and historical trends.
...
The Smart Metering Socket (SP-2W-EU) supports home energy monitoring with bidirectional metering and remote control. It uploads energy data via WiFi, allowing users to remotely control appliances and monitor usage via apps.
...
Smart DIN Rail Meter
Designed for residential/small commercial bidirectional metering. Installs on DIN rails (35mm) using split-core CTs. Uploads data via WiFi/Ethernet for detailed energy analysis.
